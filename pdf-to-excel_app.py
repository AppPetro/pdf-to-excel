...
st.markdown(
    ...
    1. Próbuje wyciągnąć tekst przez PyPDF2 lub pdfplumber.
    2. Stosuje parsowanie w układach D, E, B, C lub A.
...
def parse_layout_c(lines: list[str]) -> pd.DataFrame:
    # Układ C – dowolne 13-cyfrowe wystąpienie EAN skojarzone z kolejnym Lp i ilością.
    # Mapa indeks->EAN
    ean_map: dict[int, str] = {}
    for idx, ln in enumerate(lines):
        if match := re.search(r"\b(\d{13})\b", ln):
            ean_map[idx] = match.group(1)
    out = []
    for i, ln in enumerate(lines):
        ...
        if re.fullmatch(r"\d+", ln):
            ...
            prev_idxs = [k for k in ean_map if k < i]
            if not prev_idxs:
                continue
            symbol = ean_map[max(prev_idxs)]
            # znajdź ilość w następnych liniach
            qty = None
            for j in range(i+1, min(i+5, len(lines))):
                if m2 := re.search(r"\b(\d{1,4})\s*szt\b", lines[j], flags=re.IGNORECASE):
                    qty = int(m2.group(1))
                    break
            if qty is not None:

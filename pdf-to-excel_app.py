...
st.markdown(
    ...
    4. Wykrywamy format “WZ/Subiekt GT” i parsujemy EAN:
       - Pierwsza próba: ilość + 'szt.' + EAN gdziekolwiek dalej w tej samej linii.
...
def parse_layout_wz(all_lines: list[str]) -> pd.DataFrame:
    """
    1) Szuka patternu '<ilość> szt.' + ... + '<EAN>' w tej samej linii
    2) Jeśli nic nie znajdzie – fallback na Symbol=EAN w kolumnowej wersji.
    """
    products = []
    lp = 1

    # 1) pierwsza próba: EAN po 'szt.' gdziekolwiek dalej
    ...
    # 2) fallback: kolumnowy układ (Symbol=EAN)
    pat2 = re.compile(
        r"^(\d+)\s+"         # Lp
        r"(\d{13})\s+"       # Symbol = EAN
        r".+?\s+"            # Nazwa + inne
...
# — Twoje dotychczasowe parsery fakturowe — D, E, B, C, A — bez zmian —
...
# 1) usuwamy stopki/numerację stron
lines = [ln for ln in lines if not ln.startswith("/") and "Strona" not in ln]

# 2) wstawiamy spację między numerem a nazwą
...
# 5) filtrujemy puste ilości i sprawdzenie
...
if missing:
    ...
    st.error(f"Znaleziono w sumie: {total} pozycji  \\nUnikalnych EAN-ów: {unique}")
else:
    st.markdown(
